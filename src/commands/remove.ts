...
export default function process(message: Discord.Message): void {
    ...
    if (pos < 0) {
        notifyAuthorOfFailure(
            message,
            "You need to specify what should be removed"
        );
        return;
    }
    ...
    message.author.send(
        `Removed "${whatToRemove}" from all insult/compliment lists`
    );
}

process.help = "remove something from the insult/compliment lists";
