...
import { addMessageAuthorToChannel, removeMessageAuthorFromChannel } from "./userPermissions";
...
export async function handleRegex(message: Discord.Message) {
	...
	if (tryToPostInSameChannel(message, newText, getUserName(message.member), "Can't regex on this channel")) {
		subTarget.delete();
	}
}
